        ...
        <header>
          <div id="mySidenav" class="bg-white sidenav">
            ...
            <ul class="mt-5 pl-0 link-container">
              ...
              <li>
                <a href="./loan-requests" class="active"
                  ><svg
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
      ...
      <div class="col-12 col-lg-9 col-xl-10 px-0">
        <main class="ml-lg-2">
          ...
          <section class="mt-2 main-body mb-5">
            <div class="row mx-auto">
              <div class="col-md-12">
                <div class="card transaction-history admin-loan-requests">
                  ...
                  @foreach (@$details->pending as $loan)
                      <tr>
                        <td><small>{{$loan->created_at}}</small></td>
                        <td>{{@$controller->getUserDetails($loan->user_id)->firstname}} {{@$controller->getUserDetails($loan->user_id)->lastname}}</td>
                        <td>N{{$loan->amount}}</td>
                        <td>
                          <a href="../disburse-loan/{{$loan->id}}"><div class="bg-success cursor-pointer">Accept</div> </a>
                        </td>
                        <td>
                        <a href="../decline-loan/{{$loan->id}}"><div class="bg-danger cursor-pointer">Reject</div></a>
                        </td>
                      </tr>
                    @endforeach
                    </tbody>
              ...
              <div class="col-md-6">
                <div class="card transaction-history">
                  ...
                  <table class="table table-borderless admin-table">
                    <tbody>
                    @if(@$details->disbursed)
                    @foreach (@$details->disbursed as $loan)
                      <tr>
                      <td><small>{{$loan->created_at}}</small></td>
                        <td>{{@$controller->getUserDetails($loan->user_id)->firstname}} {{@$controller->getUserDetails($loan->user_id)->lastname}}</td>
                        <td>N{{$loan->amount}}</td>
              ...
              <div class="col-md-6">
                <div class="card transaction-history border-red">
                  <h6 class="transaction-title text-capitalize">
                    Declined Requests
                  </h6>
                  <table class="table table-borderless admin-table">
                    <tbody>
                    @if(@$details->declined)
                    @foreach ($details->declined as $loan)
                      <tr>
                      <td><small>{{$loan->created_at}}</small></td>
                        <td>{{@$controller->getUserDetails($loan->user_id)->firstname}} {{@$controller->getUserDetails($loan->user_id)->lastname}}</td>
                        <td>N{{$loan->amount}}</td>
