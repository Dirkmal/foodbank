            ...
            <div class="row mx-auto">
              <div class="col-12 col-lg-7 px-0">
                <div class="card notifications-card shopping-cart">
                  ...
                  <hr />
                  <div class="pt-4 shipment-header">
                    <h6>Shipment Details</h6>
                  </div>
                  <ul class="notifications-list cart-list order-list pt-0 mt-0">
                  @if(session('cart'))
            @foreach(session('cart') as $id => $details)
                    <li>{{$details['quantity']}}X {{$details['name']}}</li>
                  
                  @endforeach
                  @endif
                  </ul>
                  <div class="text-center see-more modify">
                    <div class="modify-info">
                      <p class="total-text">Sub Total</p>
                      <h5 id="sub-total-price" class="total-price"></h5>
                    </div>
                    <div class="modify-info">
                      <p class="total-text">Delivery Fee</p>
                      <h5 class="total-price"></h5>
                    </div>
                    <hr />
                    <div class="modify-info">
                      <p class="total-text">Total</p>
                      <h5 id="overall-total-price" class="total-price"></h5>
                    </div>
                    <form action="../place-order" method="post">
                    @csrf
                          @if(session('cart'))
                  @foreach(session('cart') as $id => $details)
            <input type="number" name="product_id[]" value="{{$details['id']}}" hidden>
            ...
            <input type="number" name="product_id[]" hidden>
                    ...
                    <div class="text-center see-more proceed">
                      ...
                      <div class="proceed-btn">
                       <a href="./cart"><button>Modify</button></a>
                      </div>
                    </div>
                  </div>
                </div>
